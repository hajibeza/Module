...
spawn(function()
    ...
    while wait(.5) do
        if Auto_God_Human then
            if not Check_Available_To_Do_Got_Human() then
                Weapon = "Melee"
                Auto_Cake_Prince = true
            end
            if Use_Remote("BuyGodhuman",true) == 0 and Check_Available_To_Do_Got_Human() then
    ...
    spawn(function()
        while wait(.5) do
            if Auto_Cake_Prince then
                pcall(function()
                    local v = workspace.Enemies:GetChildren()
                    ...
                    if not Check_Near_Mon("Cake Prince") then
                        ...
                        if Check_Near_Mon(Mon_Cake) then
                            if table.find(Mon_Cake,Mon) and Check_Available_Mon(v) then
                                Start_Attack(Mon.Name,Mon.HumanoidRootPart,function()
                                    return not Auto_Cake_Prince or not Check_Near_Mon(Mon_Cake) or not Check_Available_Mon(Mon)
                                end)
                            end
                        end
                    end
