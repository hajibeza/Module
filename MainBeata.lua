...
-- local game.Players.LocalPlayer = Player.LocalPlayer
...
function Equip_Tool(Tool)
    if Check_Tool_Inventory(Tool) then 
        local ToolHumanoid = Check_Tool_Inventory(Tool)
...
function Check_Tool_Inventory(Tool_Name)
    if game.Players.LocalPlayer.Backpack:FindFirstChild(Tool_Name) or game.Players.LocalPlayer.Character:FindFirstChild(Tool_Name) then
        return true
    end
    return false
end
                    ...
                    if not Remote.KilledIceBoss and Remote.UsedKey then
                        ...
                        if Check_Near_Mon("Ice Admiral") and not workspace.Enemies:FindFirstChild("Ice Admiral") then
                            if game:GetService("ReplicatedStorage"):FindFirstChild("Ice Admiral") then
                                repeat task.wait(.1)
                                    TP(game:GetService("ReplicatedStorage"):FindFirstChild("Ice Admiral").HumanoidRootPart.CFrame * CFrame.new(0,30,0))
                        ...
                        if not Remote_Check_Rip and Remote_Swan.FlamingoAccess then
                            ...
                            if Check_Near_Mon(Don_Swan) and not workspace.Enemies:FindFirstChild(Don_Swan) then
                                if game.ReplicatedStorage:FindFirstChild(Don_Swan) then
                                    repeat task.wait(.1)
                                        TP(game:GetService("ReplicatedStorage")[Don_Swan].HumanoidRootPart.CFrame)
                        ...
                        if Remote_Check_Rip then
                            ...
                            if Check_Near_Mon(rip_indra) and not workspace.Enemies:FindFirstChild(rip_indra) then
                                if game.ReplicatedStorage:FindFirstChild(rip_indra) then
                                    repeat task.wait(.1)
                                        TP(game:GetService("ReplicatedStorage")[rip_indra].HumanoidRootPart.CFrame)
                ...
                pcall(function()
                    ...
                    if not Remote_Check_Saber.KilledShanks then
                        ...
                        if Remote_Check_Saber.UsedTorch and not Remote_Check_Saber.UsedCup then
                            ...
                            repeat wait() Use_Remote("ProQuestProgress","FillCup",game.Players.LocalPlayer.Backpack.Cup) Use_Remote("ProQuestProgress","SickMan") until not Check_Tool_Inventory("Cup") 
                        end
        ...
        while wait() do
            if Auto_Swan_Glasses then
                ...
                if Check_Near_Mon(Don_Swan) and not workspace.Enemies:FindFirstChild(Don_Swan) then
                    if game.ReplicatedStorage:FindFirstChild(Don_Swan) then
                        repeat task.wait(.1)
                            TP(game.ReplicatedStorage:FindFirstChild(Don_Swan).HumanoidRootPart.CFrame)
            ...
            if Auto_Dragon_Trident then
                ...
                if Check_Near_Mon(Tide_Keeper) and not workspace.Enemies:FindFirstChild(Tide_Keeper) then
                    if game.ReplicatedStorage:FindFirstChild(Tide_Keeper) then
                        repeat task.wait(.1)
                            TP(game.ReplicatedStorage:FindFirstChild(Tide_Keeper).HumanoidRootPart.CFrame)
            ...
            if Auto_Soul_Reaper then
                ...
                if Check_Near_Mon(Soul_Reaper) and not workspace.Enemies:FindFirstChild(Soul_Reaper) then
                    if game.ReplicatedStorage:FindFirstChild(Soul_Reaper) then
                        repeat task.wait(.1)
                            TP(game.ReplicatedStorage:FindFirstChild(Soul_Reaper).HumanoidRootPart.CFrame)
...
task.spawn(function()
    ...
    while RunService.Stepped:Wait() do
        if #canHits > 0 then
            Controller = Rigc.activeController
            if NormalClick then
    ...
    while wait() do
        if Auto_Law_Raid then
            ...
            if game.ReplicatedStorage:FindFirstChild("Order") then
                repeat task.wait(.1)
                    TP(game.ReplicatedStorage:FindFirstChild("Order").HumanoidRootPart.CFrame)
