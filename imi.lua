...
function Check_To_Farm_Mon()
    ...
    if not GetFishTail and not GetMagmaOre and not GetMysticDroplet and not GetDragonScale then
        if not FirstSea or not ThirdSea then Use_Remote("TravelMain") end

        if FirstSea then
            Mon = {"Fishman Warrior [Lv. 375]","Fishman Commando [Lv. 400]"}
        elseif ThirdSea then
            Mon = {"Fishman Raider [Lv. 1775]","Fishman Captain [Lv. 1800]"}
        end
    end
    if GetFishTail and not GetMagmaOre and not GetMysticDroplet and not GetDragonScale then
        if not FirstSea or not SecondSea then return Use_Remote("TravelMain") end

        if FireSea then
            Mon = {"Military Soldier [Lv. 300]","Military Spy [Lv. 325]"}
        elseif SecondSea then
            Mon = {"Magma Ninja [Lv. 1175]","Lava Pirate [Lv. 1200]"}
        end
    end
    if GetFishTail and GetMagmaOre and not GetMysticDroplet and not GetDragonScale then
        if not SecondSea then return Use_Remote("TravelDressrosa") end

        Mon = {"Water Fighter [Lv. 1450]","Sea Soldier [Lv. 1425]"}
    end
    if GetFishTail and GetMagmaOre and GetMysticDroplet and not GetDragonScale then
        ...
        Mon = {"Dragon Crew Archer [Lv. 1600]","Dragon Crew Warrior [Lv. 1575]"}
    end
end
            ...
            if Local_Player.Data.Fragments.Value >= 5000 and not Raiding() then
                if Local_Player.Data.Beli.Value >= 5e+6 then
                    if string.find(Use_Remote("BuyGodhuman",true),"Bring me") then
                        ...
                        local Get_Real_Name = Remove_Lv(Mon)
