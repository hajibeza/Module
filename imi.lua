            ...
            if Auto_Farm_Level then
                ...
                if not Local_Player.PlayerGui.Main.Quest.Visible then 
                    if Double_Quest then 
                        ...
                        if MyLevel > 10 then repeat wait(.1) TP(Data[Level].CFrameQuest) until (Local_Player.Character.HumanoidRootPart.Position - Data[Level].CFrameQuest.Position).Magnitude <= 3 or not Auto_Farm_Level end
                        wait(.5)
        ...
        if #Get_Hit() > 0 then
            ...
            if Controller and Controller.equipped and (not Local_Player.PlayerGui.Main.Dialogue.Visible) and Controller.currentWeaponModel then
                if (NeedAttacking or DamageAura) then
                    ...
                    RigEvent.FireServer(RigEvent,"hit",Get_Hit(),AID3 and 3 or 2,"")
                    -- AttackSignal:Fire()
                    delay(.5,function()
                        Playing:Stop()
                    end)
